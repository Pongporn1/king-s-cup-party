...
export function CardDeck({
  ...
  if (cardsRemaining === 0) {
    return <div className="relative w-36 h-52 sm:w-44 sm:h-64 rounded-xl border-2 border-dashed border-muted-foreground/30 flex items-center justify-center">
        <span className="text-muted-foreground text-lg">หมดแล้ว</span>
      </div>;
  }
  return <div className="relative my-[70px]">
      {/* Card stack effect */}
      <div className="relative w-36 h-52 sm:w-44 sm:h-64">
        ...
        return <div key={i} className={cn("absolute w-full h-full rounded-xl transition-all duration-200 overflow-hidden", "shadow-md", isTop && !disabled && "hover:scale-[1.02] hover:-translate-y-2 cursor-pointer hover:shadow-xl")} style={{
          bottom: `${i * 2}px`,
      ...
      {showCount && <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-card border border-border text-sm font-medium shadow-lg my-[300px]">
